...
EMBER_TEST("lexer recognizes punctuation and maximal munch operators") {
    ...
    constexpr auto expectedKinds = std::array{
        ...
        TokenKind::percent,   TokenKind::equal,      TokenKind::equalEqual, TokenKind::bangEqual,
...
EMBER_TEST("single-character operators consume exactly one byte") {
    ...
    tests.expect(result.diagnostics.empty(), "single-character operators have no diagnostics");
    tests.expect(result.tokens.size() == expectedKinds.size(),
    ...
    for (std::size_t index = 0; index < expectedKinds.size(); ++index) {
        ...
        tests.expect(result.tokens[index].span ==
...
EMBER_TEST("lexer skips whitespace and line comments") {
    ...
    tests.expect(result.diagnostics.empty(), "comments and whitespace have no diagnostics");
    tests.expect(result.tokens.size() == 2, "comment input emits arrow and EOF");
    if (result.tokens.size() != 2) {
        return;
    }

    tests.expect(result.tokens[0].kind == TokenKind::arrow, "arrow remains after CRLF comment");
...
EMBER_TEST("lexer emits EOF for empty input") {
    ...
    tests.expect(result.tokens.front().kind == TokenKind::endOfFile, "empty input token is EOF");
...
EMBER_TEST("lexer recognizes every keyword") {
    const auto result = lex("fn let return if else while true false i64 f64 f32 bool void extern");
    constexpr auto expectedKinds = std::array{
        TokenKind::keywordFn,
    ...
    tests.expect(result.tokens.size() == expectedKinds.size(), "every keyword emits one token");
    if (result.tokens.size() != expectedKinds.size()) {
        return;
    }
...
EMBER_TEST("lexer recognizes ASCII identifiers") {
    ...
    tests.expect(result.tokens.size() == 7, "identifiers emit six tokens and EOF");
...
EMBER_TEST("lexer recognizes numeric literals") {
    ...
    constexpr auto expectedKinds = std::array{
        ...
        TokenKind::integerLiteral,
        TokenKind::float64Literal,
        TokenKind::float32Literal,
        TokenKind::endOfFile,
    };

    tests.expect(result.diagnostics.empty(), "numeric literals have no diagnostics");
    tests.expect(result.tokens.size() == expectedKinds.size(),
...
EMBER_TEST("lexer rejects invalid numeric literals") {
    ...
    for (const auto& testCase : cases) {
        ...
        tests.expect(result.diagnostics.size() == 1, "invalid literal has one diagnostic");
        ...
        tests.expect(diagnostic.primarySpan == SourceSpan{.source = SourceId{31},
                                                          .begin = 0,
    ...
    if (dotResult.diagnostics.size() == 1) {
        ...
        tests.expect(dotResult.diagnostics.front().primarySpan ==
...
EMBER_TEST("lexer treats signs as separate tokens") {
    ...
    tests.expect(result.diagnostics.empty(), "signed forms have no diagnostics");
    tests.expect(result.tokens.size() == expectedKinds.size(),
...
EMBER_TEST("lexer rejects unsupported characters") {
    ...
    if (asciiResult.diagnostics.size() == 1) {
        ...
        tests.expect(asciiResult.diagnostics.front().primarySpan ==
                         SourceSpan{.source = SourceId{31}, .begin = 0, .end = 1},
    ...
    if (unicodeResult.diagnostics.size() == 1) {
        ...
        tests.expect(unicodeResult.diagnostics.front().primarySpan ==
...
EMBER_TEST("lexer forwards source encoding diagnostics") {
    const SourceText source{SourceId{32},
                            "invalid.ember",
                            "\xC2"
                            "A"};
    ...
    tests.expect(result.diagnostics.size() == 1, "encoding error emits one diagnostic");
    if (result.diagnostics.size() == 1) {
        tests.expect(result.diagnostics.front().code == "E1002", "encoding error preserves E1002");
        tests.expect(result.diagnostics.front().primarySpan ==
