...
EMBER_TEST("parser builds function declarations and expression precedence") {
    const SourceText source{SourceId{41},
                            "test.ember",
    ...
    tests.expect(result.program->functions.size() == 1, "program has one function declaration");
    if (result.program->functions.size() != 1) {
        return;
    }
    const auto& function = result.program->functions.front();
    tests.expect(function.body.statements.size() == 2, "function body has declaration and return");
...
EMBER_TEST("parser returns one diagnostic without partial AST") {
    const SourceText source{SourceId{42}, "invalid.ember", "fn main() -> i64 { return 1 }"};
    const auto lexResult = Lexer{}.lex(source);
    const auto result = Parser{}.parse(source, lexResult.tokens);
...
EMBER_TEST("parser preserves parenthesized and operator spans") {
    ...
    tests.expect(body.statements.size() == 1, "function body contains the return statement");
    ...
    tests.expect(multiply->operatorSpan == SourceSpan{.source = SourceId{43},
    ...
    const auto* parenthesized = std::get_if<ParenthesizedExpression>(&unary->operand->node);
    ...
    tests.expect(parenthesized->expression->span == SourceSpan{.source = SourceId{43},
                                                               .begin = openOffset + 1,
                                                               .end = closeOffset},
                 "inner expression excludes delimiters");
    const auto* inner = std::get_if<BinaryExpression>(&parenthesized->expression->node);
    tests.expect(inner != nullptr && inner->operatorSpan == SourceSpan{.source = SourceId{43},
                                                                       .begin = openOffset + 3,
                                                                       .end = openOffset + 4},
                 "inner binary expression preserves its operator span");
    tests.expect(unary->operand->span == SourceSpan{.source = SourceId{43},
                                                    .begin = openOffset,
                                                    .end = closeOffset + 1},
...
EMBER_TEST("parser accepts v0.1 statement forms") {
    const SourceText source{SourceId{44},
                            "forms.ember",
...
EMBER_TEST("parser diagnostics preserve the unexpected token span") {
    const SourceText source{SourceId{46}, "invalid.ember", "fn main() -> missing {}"};
    const auto lexResult = Lexer{}.lex(source);
    const auto result = Parser{}.parse(source, lexResult.tokens);
