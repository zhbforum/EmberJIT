...
compileAndVerify(std::string sourceText) -> std::optional<ember::bytecode::VerifiedProgram> {
    ember::support::SourceText source{ember::support::SourceId{91},
                                      "ir.ember",
                                      std::move(sourceText)};
    ...
    const auto verified = ember::bytecode::Verifier{}.verify(
        {.functions = {
             ...
             {.id = 8,
              ...
              .code = {{.opcode = ember::bytecode::Opcode::load,
                        .operand = 0,
                        .value = std::nullopt},
                       {.opcode = ember::bytecode::Opcode::returnValue,
                        .operand = 0,
...
EMBER_TEST("IR lowering creates explicit CFG blocks and virtual registers") {
    ...
    tests.expect(function.valueTypes.size() == 8,
                 "each stack value becomes a typed virtual register");
...
EMBER_TEST("IR lowering uses a one-time preheader for parameter initialization") {
    auto bytecode =
        compileAndVerify("fn countdown(n: i64) -> i64 { while n > 0 { n = n - 1; } return n; }");
    if (!bytecode) {
    ...
    for (const auto& block : blocks) {
        ...
        if (terminator.kind == TerminatorKind::branch) {
            backedgeToBytecodeEntry =
                backedgeToBytecodeEntry || (block.id != 0 && terminator.trueTarget == 1);
            hasEntryPredecessor = hasEntryPredecessor || terminator.trueTarget == 0;
        } else if (terminator.kind == TerminatorKind::branchIfFalse) {
            hasEntryPredecessor =
                hasEntryPredecessor || terminator.falseTarget == 0 || terminator.trueTarget == 0;
        }
    }
    tests.expect(backedgeToBytecodeEntry,
                 "loop backedge targets bytecode entry rather than preheader");
...
EMBER_TEST("IR verifier validates every terminator and its canonical operands") {
    const auto malformedDeadBlock =
        baseI64Function({{.id = 0,
                        ...
                        {Type::i64});
    tests.expect(rejects(malformedDeadBlock),
                 "unreachable blocks still receive structural validation");
    ...
    const auto missingTerminator = baseI64Function(
        {{.id = 0, .instructions = {Instruction::constantI64(0, 1)}, .terminator = {}}},
        {Type::i64});
    tests.expect(rejects(missingTerminator), "default-initialized terminator is invalid");

    const auto validDeadBlock = baseI64Function({{.id = 0,
                                                  .instructions = {Instruction::constantI64(0, 1)},
                                                  .terminator = Terminator::returnValue(0)},
                                                 {.id = 1,
                                                ...
                                                {Type::i64, Type::i64});
    tests.expect(rejects(validDeadBlock),
                 "structurally valid but unreachable IR block is rejected");

    const auto entryBackedge = baseI64Function({{.id = 0,
                                                 .instructions = {Instruction::constantI64(0, 1)},
                                               ...
                                               {Type::i64, Type::i64});
    tests.expect(rejects(entryBackedge),
                 "entry block predecessor is rejected directly by the verifier");
}

EMBER_TEST("IR verifier rejects malformed instruction encodings and definitions") {
    const auto nonCanonical = baseI64Function({{.id = 0,
    ...
    const auto nonDense = baseI64Function({{.id = 0,
    ...
    const auto duplicateDefinition = baseI64Function(
        {{.id = 0,
          ...
          .terminator = Terminator::returnValue(0)}},
        {Type::i64});
    tests.expect(rejects(duplicateDefinition),
                 "duplicate virtual-register definitions are rejected");

    const auto parameterOutsideEntry =
        Function{.id = 0,
                 .signature = {.parameterTypes = {Type::i64}, .returnType = Type::i64},
                 .localTypes = {Type::i64},
...
EMBER_TEST("IR verifier checks return types and local initialization merges") {
    auto wrongReturn = baseI64Function({{.id = 0,
                                         .instructions = {Instruction::constantI64(0, 1)},
                                         .terminator = Terminator::returnValue(0)}},
                                       {Type::i64});
    ...
    const auto partialInitialization = baseI64Function(
        ...
        {Type::i64, Type::i64, Type::boolean, Type::i64, Type::i64},
        {Type::i64});
    tests.expect(rejects(partialInitialization),
                 "local initialized on one merge edge is not definite");
    ...
    const auto completeInitialization = baseI64Function(
        ...
        {Type::i64});
    tests.expect(!rejects(completeInitialization),
                 "local initialized on every merge edge verifies");
}
...
EMBER_TEST("IR verifier enforces trusted typed call contracts") {
    ...
    const auto rejectsCall = [&callTargets](Function function) {
        return !ember::ir::Verifier{}
                    .verify(std::move(function), *callTargets)
                    .function.has_value();
    };
    ...
    auto nonCallMetadata = baseI64Function({{.id = 0,
                                             .instructions = {Instruction::constantI64(0, 1)},
                                             .terminator = Terminator::returnValue(0)}},
                                           {Type::i64});
        ...
        ember::ir::Verifier{}
            .verify(
                {.id = 0,
                 ...
                 .localTypes = {},
                 .valueTypes = {},
                 .blocks = {{.id = 0, .instructions = {}, .terminator = Terminator::returnVoid()}}})
            .function.has_value(),
...
EMBER_TEST("IR lowering materializes every native v0.1 value form") {
    ...
    auto boolProgram =
        compileAndVerify("fn main() -> i64 { if true { return 1; } else { return 0; } }");
    if (!boolProgram) {
    ...
    auto f64LocalProgram = compileAndVerify("fn main() -> i64 { let value: f64 = 1.0; return 1; }");
    ...
    tests.expect(!invalidId.function &&
                     invalidId.failure == ember::ir::LoweringFailure::invalidInput,
...
EMBER_TEST("IR constant folding preserves wrapping i64 semantics and re-verifies its output") {
    ...
    tests.expect(folded.function.has_value(),
                 "constant folding postcondition passes the IR verifier");
    if (!folded.function)
        return;
    ...
    auto divisionFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 1),
                                           Instruction::constantI64(1, 0),
    ...
    tests.expect(divisionFolded.function.has_value() &&
                     divisionFolded.function->function().blocks[0].instructions[2].opcode ==
                         Opcode::divI64,
                 "constant folding retains division so its runtime error remains observable");

    auto remainderFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 1),
                                           Instruction::constantI64(1, 0),
    ...
    tests.expect(remainderFolded.function.has_value() &&
                     remainderFolded.function->function().blocks[0].instructions[2].opcode ==
                         Opcode::remI64,
                 "constant folding retains remainder so its runtime error remains observable");

    auto comparisonFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 2),
                                           Instruction::constantI64(1, 3),
                                           Instruction::binary(Opcode::lessI64, 2, 0, 1),
    ...
    tests.expect(comparisonFolded.function.has_value() &&
                     comparisonFolded.function->function().blocks[0].instructions[2].opcode ==
                         Opcode::lessI64,
...
EMBER_TEST("IR propagation and DCE retain required effects while removing dead pure values") {
    ...
    auto propagationFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 7),
                                           Instruction::storeLocal(0, 0),
                                           Instruction::loadLocal(1, 0)},
                          .terminator = Terminator::returnValue(1)}},
                        {Type::i64, Type::i64},
    ...
    tests.expect(propagated.function.has_value() &&
                     propagated.function->function().blocks[0].instructions[2].opcode ==
                         Opcode::constantI64,
                 "local constant propagation replaces a same-block load and re-verifies IR");

    auto overwriteFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 0),
                                           Instruction::storeLocal(0, 0),
                                           Instruction::constantI64(1, 7),
                                           Instruction::storeLocal(0, 1),
                                           Instruction::constantI64(2, 9),
                                           Instruction::storeLocal(1, 2),
                                           Instruction::loadLocal(3, 0),
                                           Instruction::loadLocal(4, 1)},
                          .terminator = Terminator::returnValue(3)}},
    ...
    auto invalidationFixture = ember::ir::Verifier{}.verify(
        {.id = 0,
         ...
         .blocks = {{.id = 0,
                     ...
                     .terminator = Terminator::returnValue(2)}}});
    tests.expect(invalidationFixture.function.has_value(),
                 "nonconstant-store propagation fixture verifies");
    if (!invalidationFixture.function)
        return;
    const auto invalidated =
        ember::ir::ConstantPropagationPass{}.run(*invalidationFixture.function);
    tests.expect(invalidated.function.has_value() &&
                     invalidated.function->function().blocks[0].instructions[4].opcode ==
                         Opcode::loadLocal,
    ...
    tests.expect(edgePropagated.function.has_value() &&
                     edgePropagated.function->function().blocks[1].instructions[0].opcode ==
                         Opcode::loadLocal,
                 "constant propagation does not cross CFG edges");

    auto refoldingFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 2),
                                           Instruction::storeLocal(0, 0),
                                           Instruction::loadLocal(1, 0),
                        ...
                        {Type::i64, Type::i64, Type::i64, Type::i64},
                        {Type::i64}));
    tests.expect(refoldingFixture.function.has_value(),
                 "post-propagation folding fixture verifies");
    ...
    tests.expect(eliminated.function.has_value() &&
                     eliminated.function->function().valueTypes.size() == 1 &&
    ...
    auto effectFixture = ember::ir::Verifier{}.verify(
        baseI64Function({{.id = 0,
                          .instructions = {Instruction::constantI64(0, 1),
                                           Instruction::constantI64(1, 0),
                                           Instruction::binary(Opcode::divI64, 2, 0, 1),
                                           Instruction::binary(Opcode::remI64, 3, 0, 1),
                                           Instruction::callI64(4, 7, {0}),
                                           Instruction::constantI64(5, 42)},
                          .terminator = Terminator::returnValue(5)}},
    ...
    tests.expect(
        effectsRetained.function.has_value() &&
    ...
    auto remapFixture = ember::ir::Verifier{}.verify(
        baseI64Function(
            {{.id = 0,
              .instructions = {Instruction::constantI64(0, 99),
                               Instruction::constantI64(1, 7),
                               Instruction::constantI64(2, 8),
                               Instruction::binary(Opcode::addI64, 3, 1, 2),
                               Instruction::storeLocal(0, 3),
                               Instruction::loadLocal(4, 0),
                               Instruction::callI64(5, 8, {4, 1}),
                               Instruction::constantI64(6, 42)},
    ...
    tests.expect(remapped.function.has_value() &&
                     remapped.function->function().valueTypes.size() == 6 &&
    ...
    tests.expect(eliminatedAgain.function.has_value() &&
                     ember::ir::dump(*eliminatedAgain.function) ==
                         ember::ir::dump(*remapped.function),
                 "DCE is idempotent");
}

EMBER_TEST(
    "IR default optimization pipeline simplifies constant CFG and exposes the result in dumps") {
    ...
    tests.expect(optimized.function.has_value(),
                 "every default pass preserves the verifier contract");
    if (!optimized.function)
        return;
    const auto& function = optimized.function->function();
    const auto dump = ember::ir::dump(*optimized.function);
    tests.expect(function.blocks.size() == 2 &&
    ...
    tests.expect(optimizedAgain.function.has_value() && ember::ir::dump(*optimizedAgain.function) ==
                                                            ember::ir::dump(*optimized.function),
...
EMBER_TEST("CFG simplification evaluates every i64 comparison only in branch conditions") {
    const auto outcome =
        [](Opcode opcode, std::int64_t left, std::int64_t right) -> std::optional<std::int64_t> {
        auto verified = ember::ir::Verifier{}.verify(
            baseI64Function({{.id = 0,
                              .instructions = {Instruction::constantI64(0, left),
                                               Instruction::constantI64(1, right),
    ...
    tests.expect(
        outcome(Opcode::equalI64, 4, 4) == 11 && outcome(Opcode::equalI64, 4, 5) == 22 &&
            ...
            outcome(Opcode::lessEqualI64, 4, 4) == 11 &&
            outcome(Opcode::lessEqualI64, 5, 4) == 22 && outcome(Opcode::greaterI64, 5, 4) == 11 &&
            outcome(Opcode::greaterI64, 4, 5) == 22 &&
