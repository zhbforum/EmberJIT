...
{
    const SourceText source{SourceId{81}, "host.ember",
...
EMBER_TEST("semantic analyzer handles mutual recursion and host registry failures")
{
    ...
    const bool first = hosts.add(
        HostFunction{.name = "host",
                     .signature = {.parameterTypes = {Type::i64}, .returnType = Type::voidType}});
    const bool duplicate = hosts.add(HostFunction{
        .name = "host", .signature = {.parameterTypes = {}, .returnType = Type::voidType}});
    tests.expect(first && !duplicate, "host registry rejects duplicate names");

    const SourceText collisionSource{SourceId{83}, "collision.ember",
                                     "fn host() -> void { return; }"};
    ...
    const SourceText invalidSource{SourceId{84}, "invalid_host.ember",
                                   "fn main() -> void { return; }"};
