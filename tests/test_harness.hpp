...
{
  public:
    void beginTest(std::string_view name) noexcept { currentTest_ = name; }
                ...
                std::source_location location = std::source_location::current())
    {
        ...
        std::cerr << location.file_name() << ':' << location.line() << ": ["
                  << currentTest_ << "] expectation failed: " << expression
                  << '\n';
    }
...
class TestRegistry
{
  public:
    ...
    void add(TestCase testCase) { testCases_.push_back(testCase); }
  ...
  public:
    TestRegistration(std::string_view name, TestFunction function)
    {
        TestRegistry::instance().add(
            TestCase{.name = name, .function = function});
    }
};
...
#define EMBER_TEST_IMPL(name, line)                                            \
    static void EMBER_TEST_JOIN(emberTest_,                                    \
                                line)(::ember::test::TestContext & tests);     \
    [[maybe_unused]] static const ::ember::test::TestRegistration              \
        EMBER_TEST_JOIN(emberRegistration_,                                    \
