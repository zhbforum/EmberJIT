...
EMBER_TEST("x64 emitter encodes arithmetic with golden bytes") {
    ...
    constexpr std::array expected{std::byte{0x48},
                                  std::byte{0x89},
                                  std::byte{0xC8},
                                  std::byte{0x48},
                                  std::byte{0x01},
...
EMBER_TEST("x64 emitter resolves forward rel32 branch fixups") {
    ...
    if (emitted.code)
        tests.expect(isEqual(emitted.code->bytes(), expected),
                     "forward rel32 displacement is stable");
}
...
EMBER_TEST("x64 emitter resolves unconditional rel32 branch fixups") {
    ...
    auto emitted = emitter.finalize();
    constexpr std::array expected{std::byte{0xE9},
                                  std::byte{0x00},
                                  std::byte{0x00},
                                  std::byte{0x00},
                                  std::byte{0x00},
...
EMBER_TEST("x64 emitter encodes the remaining integer leaf subset") {
    ember::jit::x64::Emitter emitter;
    tests.expect(
        emitter.moveImmediate64(ember::jit::x64::Register::r8, -1) &&
            ...
            emitter.signedDivide(ember::jit::x64::Register::r10) && emitter.returnFromFunction(),
...
EMBER_TEST("x64 emitter uses a REX prefix for every low-byte setcc register") {
    ember::jit::x64::Emitter emitter;
    tests.expect(
        emitter.set(ember::jit::x64::Condition::equal, ember::jit::x64::Register::rsp) &&
    ...
    constexpr std::array expected{std::byte{0x40},
                                  std::byte{0x0F},
                                  std::byte{0x94},
                                  std::byte{0xC4},
                                  std::byte{0x41},
...
EMBER_TEST("x64 emitter rejects implicit division registers") {
    ...
    tests.expect(!emitted.code &&
                     emitted.error == ember::jit::x64::EmitError::invalidDivisionOperand,
...
EMBER_TEST("native code handle rejects an empty publication") {
    ember::jit::x64::Emitter emitter;
    auto emitted = emitter.finalize();
    tests.expect(emitted.code.has_value(),
                 "empty emitter can produce an empty finalized byte sequence");
    if (!emitted.code)
        return;

    const auto native =
        ember::runtime::NativeCodeHandle::publishI64Binary(std::move(*emitted.code));
...
EMBER_TEST("x64 emitter encodes stack-frame memory and indirect-call primitives") {
    ember::jit::x64::Emitter emitter;
    tests.expect(
        emitter.push(ember::jit::x64::Register::rbx) && emitter.subtractStackPointer(32) &&
            emitter.load(ember::jit::x64::Register::rax, ember::jit::x64::Register::rsp, 16) &&
            emitter.loadEffectiveAddress(ember::jit::x64::Register::r8,
                                         ember::jit::x64::Register::rsp,
                                         40) &&
            emitter.store(ember::jit::x64::Register::rsp, 24, ember::jit::x64::Register::rax) &&
    ...
    if (emitted.code) {
        constexpr std::array expectedLea{std::byte{0x4C},
                                         std::byte{0x8D},
                                         std::byte{0x84},
                                         std::byte{0x24},
                                         std::byte{0x28},
...
EMBER_TEST("native code handle executes Win64 i64 arithmetic from RX memory") {
    ...
    const auto queryResult =
        ::VirtualQuery(ember::runtime::test::NativeCodeHandleAccess::entryAddress(moved),
                       &memoryInfo,
                       sizeof(memoryInfo));
    tests.expect(queryResult == sizeof(memoryInfo),
                 "published code has a queryable virtual-memory region");
...
EMBER_TEST("native code handle executes both generated branch paths") {
    ...
    tests.expect(native.handle->invokeI64Binary(5, 5) == 5, "generated branch handles equality");
}

EMBER_TEST("native code handle executes ordered SSE2 f64 comparisons") {
    ember::jit::x64::Emitter emitter;
    tests.expect(
        emitter.moveToXmm(ember::jit::x64::XmmRegister::xmm0, ember::jit::x64::Register::rcx) &&
            emitter.moveToXmm(ember::jit::x64::XmmRegister::xmm1, ember::jit::x64::Register::rdx) &&
            ...
            emitter.set(ember::jit::x64::Condition::below, ember::jit::x64::Register::rax) &&
            emitter.returnFromFunction(),
...
EMBER_TEST("native frame adapter executes generated local-slot access") {
    ember::jit::x64::Emitter emitter;
    tests.expect(
        emitter.load(ember::jit::x64::Register::rax, ember::jit::x64::Register::rcx, 0) &&
