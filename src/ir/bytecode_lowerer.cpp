...
[[nodiscard]] bool isComparison(Opcode opcode) noexcept {
    return isComparisonI64(opcode) || opcode == Opcode::equalF64 || opcode == Opcode::notEqualF64 ||
           opcode == Opcode::lessF64 || opcode == Opcode::lessEqualF64 ||
           opcode == Opcode::greaterF64 || opcode == Opcode::greaterEqualF64 ||
           opcode == Opcode::equalBool || opcode == Opcode::notEqualBool;
}
} // namespace
...
LoweringResult Lowerer::lower(const bytecode::VerifiedProgram& program,
    ...
    const auto found = std::find_if(
        functions.begin(),
        functions.end(),
        [functionId](const bytecode::Function& function) { return function.id == functionId; });
    ...
    if (source.code.empty()) {
        return {.function = std::nullopt,
                ...
                .diagnostics = {
                    loweringError(LoweringFailure::internalInvariant,
                                  "function #" + std::to_string(source.id) +
    ...
    for (std::size_t parameter = 0; parameter < source.signature.parameterTypes.size();
         ++parameter) {
        const auto value = makeValue(source.signature.parameterTypes[parameter]);
        parameterValues.push_back(value);
        preheader.instructions.push_back(
            Instruction::parameter(value, static_cast<LocalId>(parameter)));
    }
    ...
    for (std::size_t blockIndex = 0; blockIndex < leaders.size(); ++blockIndex) {
        ...
        const auto end =
            blockIndex + 1 < leaders.size() ? leaders[blockIndex + 1] : source.code.size();
                ...
                if (std::holds_alternative<std::int64_t>(*instruction.value))
                    block.instructions.push_back(
                        Instruction::constantI64(value,
                                                 std::get<std::int64_t>(*instruction.value)));
            ...
            if (instruction.opcode == bytecode::Opcode::call) {
                const auto callee =
                    std::find_if(functions.begin(),
                                 functions.end(),
                                 [&instruction](const bytecode::Function& function) {
                ...
                for (std::size_t index = arguments.size(); index > 0; --index) {
                    ...
                    if (!argument || function.valueTypes[*argument] !=
                                         callee->signature.parameterTypes[index - 1])
                        return fail(
                            LoweringFailure::internalInvariant,
            ...
            if (instruction.opcode == bytecode::Opcode::jumpIfFalse) {
                ...
                if (!condition || function.valueTypes[*condition] != semantic::Type::boolean)
                    return fail(LoweringFailure::internalInvariant,
                                atPc(source.id,
                                     pc,
                                     "verified bytecode has an invalid conditional jump state"));
                if (!blockAt[instruction.operand] || blockIndex + 1 >= leaders.size())
                    return fail(LoweringFailure::internalInvariant,
                                atPc(source.id,
                                     pc,
                                     "verified bytecode has an invalid conditional jump target"));
                if (!stack.empty())
                    return fail(
                        LoweringFailure::unsupported,
                        atPc(source.id,
                             pc,
                             "non-empty operand stack at a CFG edge is outside the native subset"));
                block.terminator = Terminator::branchIfFalse(*condition,
                                                             *blockAt[instruction.operand],
                                                             static_cast<BlockId>(blockIndex + 2));
                terminated = true;
                continue;
            }
            ...
            if (instruction.opcode == bytecode::Opcode::returnVoid) {
                if (!stack.empty())
                    return fail(LoweringFailure::internalInvariant,
                                atPc(source.id,
                                     pc,
                                     "verified bytecode returns void with a non-empty stack"));
    ...
    diagnostics.insert(diagnostics.end(),
                       std::make_move_iterator(verified.diagnostics.begin()),
