...
struct ConstantFacts {
    explicit ConstantFacts(std::size_t valueCount)
        : i64(valueCount),
          boolean(valueCount) {
    }
    ...
    for (auto& block : function.blocks) {
        for (auto& instruction : block.instructions) {
            if (instruction.opcode == Opcode::negateI64 && facts.i64[instruction.result])
                instruction =
                    Instruction::constantI64(instruction.result, *facts.i64[instruction.result]);
        ...
        for (auto& instruction : block.instructions) {
            if (instruction.opcode == Opcode::constantI64)
                values[instruction.result] = instruction.constant;
                     ...
                     locals[instruction.local].epoch == epoch && locals[instruction.local].value) {
...
VerifyResult DeadCodeEliminationPass::run(const VerifiedFunction& input) const {
    ...
    for (auto& block : function.blocks) {
        ...
        for (auto iterator = block.instructions.rbegin(); iterator != block.instructions.rend();
             ++iterator) {
