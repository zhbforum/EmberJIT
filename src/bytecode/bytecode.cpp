...
class FunctionCompiler
{
  ...
  private:
    ...
    void statementNode(const semantic::TypedBlockPtr &node) { block(*node); }
...
VerifyResult Verifier::verify(Program program) const
{
    ...
    const auto instructionError = [&](const Function &function, std::size_t pc, std::string message)
    {
        ...
        while (!work.empty())
        {
            ...
            case Opcode::call:
            {
                ...
                for (auto it = signature.parameterTypes.rbegin();
                     it != signature.parameterTypes.rend(); ++it)
                    valid = pop(*it) && valid;
