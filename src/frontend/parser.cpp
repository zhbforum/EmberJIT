...
[[nodiscard]] auto joinSpans(support::SourceSpan first,
                             support::SourceSpan last) noexcept
    -> support::SourceSpan
{
                                  ...
                                  std::string message) -> support::Diagnostic
{
    return support::Diagnostic{
        ...
        .primarySpan = support::SourceSpan{
            .source = source,
            .begin = begin,
            .end = end,
        },
...
class ParserState
{
  ...
  private:
    ...
    [[nodiscard]] auto consume(TokenKind kind, std::string message)
        -> const Token *
    {
        if (!at(kind))
        {
    ...
    void fail(std::string code, std::string message)
    {
        ...
        diagnostics_.push_back(makeDiagnostic(
            source_.id(), token.span.begin, token.span.end, std::move(code),
            std::move(message)));
    }

    void failAtEnd(std::string code, std::string message)
    {
        diagnostics_.push_back(makeDiagnostic(source_.id(), source_.size(),
                                              source_.size(), std::move(code),
                                              std::move(message)));
    }
    ...
    [[nodiscard]] auto parseParameter() -> std::optional<Parameter>
    {
        ...
        if (name == nullptr || consume(TokenKind::colon, "expected ':' after parameter") == nullptr)
        {
            return std::nullopt;
        }
    ...
    [[nodiscard]] auto parseType()
        -> std::optional<std::pair<TypeName, support::SourceSpan>>
    {
        const auto token = current();
        TypeName type{};
        switch (token.kind)
    ...
    [[nodiscard]] auto parseAssignment() -> std::optional<Statement>
    {
        ...
        return Statement{.span = joinSpans(target.span, semicolon->span),
                         .node = AssignmentStatement{.targetSpan = target.span,
    ...
    [[nodiscard]] auto parseWhile() -> std::optional<Statement>
    {
        ...
        return Statement{.span = joinSpans(keyword.span, body->span),
                         .node = WhileStatement{.condition = std::move(condition),
                                                .body = std::move(body)}};
    }

    [[nodiscard]] auto parseExpression() -> ExpressionPtr { return parseEquality(); }
    ...
    [[nodiscard]] auto parseEquality() -> ExpressionPtr
    {
        ...
        while (expression != nullptr && (at(TokenKind::equalEqual) || at(TokenKind::bangEqual)))
        {
            ...
            expression = binary(operation, operatorToken.span, std::move(expression),
                                std::move(right));
        }
        return expression;
    }
    ...
    [[nodiscard]] auto parseTerm() -> ExpressionPtr
    {
        ...
        {
            const auto &operatorToken = advance();
            const auto operation = operatorToken.kind == TokenKind::plus
                                       ? BinaryOperator::add
...
auto Parser::parse(const support::SourceText &source, std::span<const Token> tokens) const
    -> ParseResult
{
