...
[[nodiscard]] auto typeName(TypeName type) noexcept -> std::string_view
{
    switch (type)
    {
    case TypeName::i64: return "i64";
    case TypeName::f64: return "f64";
    case TypeName::boolean: return "bool";
    case TypeName::voidType: return "void";
    }
    return "<invalid-type>";
}
...
class Printer
{
  ...
  private:
    ...
    {
        output_ << std::string(indent_ * 2, ' ') << name << " [" << span.begin
                << ", " << span.end << ")\n";
    }
    ...
    {
        output_ << std::string(indent_ * 2, ' ') << "Function "
                << text(function.nameSpan) << " -> " << typeName(function.returnType)
                << " [" << function.span.begin << ", " << function.span.end << ")\n";
        ++indent_;
        for (const auto &parameter : function.parameters)
        {
            output_ << std::string(indent_ * 2, ' ') << "Parameter "
                    << text(parameter.nameSpan) << ": " << typeName(parameter.type)
    ...
    {
        output_ << std::string(indent_ * 2, ' ') << "Assignment "
                << text(node.targetSpan) << " [" << span.begin << ", " << span.end << ")\n";
        ++indent_;
    ...
    void printExpressionNode(const ParenthesizedExpression &node,
...
auto AstPrinter::print(const Program &program, const support::SourceText &source) const
    -> std::string
{
