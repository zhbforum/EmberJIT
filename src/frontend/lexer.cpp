...
[[nodiscard]] constexpr auto utf8Length(std::uint8_t first) noexcept
    -> std::size_t
{
    if (first <= 0x7FU)
    {
...
[[nodiscard]] constexpr auto isNumericCandidateCharacter(char value) noexcept
    -> bool
{
...
[[nodiscard]] constexpr auto isValidInteger(std::string_view text) noexcept
    -> bool
{
...
[[nodiscard]] constexpr auto classifyNumber(std::string_view text) noexcept
    -> std::optional<TokenKind>
{
    ...
    const auto fractionPart =
        text.substr(decimalPoint + 1, fractionEnd - decimalPoint - 1);
...
[[nodiscard]] constexpr auto classifyIdentifier(std::string_view text) noexcept
    -> TokenKind
{
    if (text == "fn")
    {
...
[[nodiscard]] auto makeDiagnostic(support::SourceId source, std::size_t begin,
                                  std::size_t end, std::string code,
                                  std::string message) -> support::Diagnostic
{
    return support::Diagnostic{
        ...
        .primarySpan =
            support::SourceSpan{.source = source, .begin = begin, .end = end},
    };
}
...
class Scanner
{
    ...
    [[nodiscard]] LexResult scan()
    {
        while (!atEnd())
        {
            ...
            default:
                ...
                {
                    return error(begin, begin + utf8Length(asByte(character)),
  ...
  private:
    ...
    [[nodiscard]] char current() const noexcept { return text_[offset_]; }
    ...
    [[nodiscard]] bool scanNumber(std::size_t begin)
    {
        ...
        if (literal.size() > 1 && literal.front() == '0' &&
            isAsciiDigit(literal[1]))
        {
            addDiagnostic(begin, offset_, "E1005",
    ...
    void emit(TokenKind kind, std::size_t begin, std::size_t end)
    {
        result_.tokens.push_back(Token{
            .kind = kind,
            .span = support::SourceSpan{.source = source_.id(),
                                        .begin = begin,
                                        .end = end},
        });
    }

    [[nodiscard]] LexResult error(std::size_t begin, std::size_t end,
                                  std::string message)
    {
    ...
    void addDiagnostic(std::size_t begin, std::size_t end, std::string code,
                       std::string message)
    {
        result_.diagnostics.push_back(makeDiagnostic(
            source_.id(), begin, end, std::move(code), std::move(message)));
    }
...
{
    if (const auto diagnostic = source.validateEncoding();
