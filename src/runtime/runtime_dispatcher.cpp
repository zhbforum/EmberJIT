...
RuntimeDispatcher::dispatch(semantic::FunctionId functionId,
    ...
    const bool becameHot =
        options_.profilingEnabled && function->recordInvocation(options_.hotThreshold);
    if (becameHot) {
        events_.push_back({.functionId = function->id(),
                           .invocationCount = function->profiling().invocationCount});
