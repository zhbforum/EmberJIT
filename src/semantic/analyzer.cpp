...
private:
    ...
    [[nodiscard]] auto validSignature(const FunctionSignature& signature,
    ...
    class ScopeGuard {
    public:
        explicit ScopeGuard(Analyzer& analyzer)
            : analyzer_(analyzer) {
    ...
    [[nodiscard]] auto
    declare(support::SourceSpan span, std::string name, Type type) -> std::optional<Symbol> {
    ...
    [[nodiscard]] auto
    analyzeStatement(const frontend::Statement& statement) -> std::optional<TypedStatement> {
        return std::visit(
    ...
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::LetStatement& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
    ...
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::AssignmentStatement& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
    ...
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::ReturnStatement& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
    ...
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::IfStatement& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
    ...
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::WhileStatement& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
        auto condition = analyzeExpression(*node.condition);
        if (condition == nullptr) {
            return std::nullopt;
    ...
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::ExpressionStatement& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
        ...
        return TypedStatement{.span = span,
                              .node =
                                  TypedExpressionStatement{.expression = std::move(expression)}};
    }
    [[nodiscard]] auto
    analyzeStatementNode(const frontend::BlockPtr& node,
                         support::SourceSpan span) -> std::optional<TypedStatement> {
    ...
    [[nodiscard]] auto
    analyzeExpression(const frontend::Expression& expression) -> TypedExpressionPtr {
        return std::visit(
    ...
    [[nodiscard]] auto
    makeExpression(support::SourceSpan span, Type type, auto node) -> TypedExpressionPtr {
    ...
    [[nodiscard]] auto analyzeExpressionNode(const frontend::CallExpression& node,
        ...
        return makeExpression(span,
                              found->second.signature.returnType,
    ...
    [[nodiscard]] auto
    parseIntegerLiteral(support::SourceSpan literalSpan,
    ...
    [[nodiscard]] auto parseFloatLiteral(support::SourceSpan literalSpan,
        ...
        return makeExpression(expressionSpan,
                              Type::f64,
                              TypedLiteralExpression{.literalSpan = literalSpan, .value = value});
    }

    [[nodiscard]] auto
    tryFoldMinimumI64(const frontend::UnaryExpression& node,
    ...
    [[nodiscard]] auto
    checkBinaryOperator(frontend::BinaryOperator operation,
                        Type left,
                        Type right,
                        support::SourceSpan operatorSpan) -> std::optional<Type> {
    ...
    [[nodiscard]] auto
    integerLiteralMagnitude(support::SourceSpan span) -> std::optional<std::uint64_t> {
        std::uint64_t value{};
...
auto SemanticAnalyzer::analyze(const frontend::Program& program,
