...
[[nodiscard]] bool isCanonicalPreheader(const ir::Function &function, const ir::BasicBlock &preheader)
{
    ...
    for (std::size_t index{}; index < parameterCount; ++index)
    {
        ...
        if (parameter.opcode != ir::Opcode::parameter || parameter.result != index ||
            ...
            store.opcode != ir::Opcode::storeLocal ||
            store.result != ir::noValue || store.input != index || store.left != ir::noValue ||
            store.right != ir::noValue || store.local != index || store.constant != 0 ||
            store.callee != ir::noFunction || store.calleeKind != semantic::FunctionKind::user ||
            !store.arguments.empty())
            return false;
    }
    return true;
}
...
BaselineCompileResult BaselineCompiler::compile(const ir::VerifiedFunction &verified) const
{
    ...
    for (const auto &block : function.blocks)
        for (const auto &instruction : block.instructions)
            if (instruction.opcode == ir::Opcode::callI64 || instruction.opcode == ir::Opcode::callValue ||
    ...
    const auto callMatchesTrustedTarget = [&function, &verified](const ir::Instruction &instruction)
    {
        ...
        if (voidCall != (target->signature.returnType == semantic::Type::voidType) ||
            (!voidCall && (instruction.result == ir::noValue ||
    ...
    {
        return emitter.moveImmediate64(Register::rax, 0) && emitter.addStackPointer(callAreaBytes) &&
    ...
    for (std::size_t blockIndex = 1; blockIndex < function.blocks.size(); ++blockIndex)
    {
        ...
        for (const auto &instruction : block.instructions)
        {
            ...
            case ir::Opcode::negateF64:
            {
                ...
                const auto inputOffset = valueOffset(instruction.input);
                if (!resultOffset || !inputOffset || !emitter.load(Register::rax, Register::r13, *inputOffset) ||
                    !emitter.moveImmediate64(Register::r8, std::numeric_limits<std::int64_t>::min()) ||
            ...
            case ir::Opcode::callVoid:
            {
                ...
                if (instruction.calleeKind == semantic::FunctionKind::host)
                {
                    ...
                    case bytecode::NativeBuiltinAbi::i64ToVoid:
                    {
                        ...
                        if (!argumentOffset || !emitter.load(Register::rcx, Register::r13, *argumentOffset) ||
                    ...
                    {
                        const auto resultOffset = valueOffset(instruction.result);
                        if (!resultOffset || !emitter.moveImmediate64(Register::rax, entryAddress) ||
                            !emitter.call(Register::rax) ||
                            !emitter.store(Register::r13, *resultOffset, Register::rax))
                            return {.code = std::nullopt,
                                    .error = BaselineCompileError::emissionFailed};
