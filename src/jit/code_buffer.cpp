...
CodeBufferCreateResult CodeBuffer::create(std::span<const std::byte> code,
                                          TestHooks testHooks) noexcept {
    if (code.empty())
        return {.buffer = std::nullopt, .error = CodeBufferError::emptyCode};

#if EMBER_HAS_WIN64_JIT
    void* const memory =
        ::VirtualAlloc(nullptr, code.size(), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (memory == nullptr)
