...
Emitter::Emitter()
    : identity_(std::make_shared<EmitterIdentity>()) {
}
...
bool Emitter::moveToXmm(XmmRegister destination, Register source) {
    ...
    appendByte(0x0F);
    appendByte(0x6E);
    appendByte(static_cast<std::uint8_t>(0xC0U |
                                         (static_cast<std::uint32_t>(destinationCode) << 3U) |
...
bool Emitter::moveFromXmm(Register destination, XmmRegister source) {
    ...
    appendByte(0x7E);
    appendByte(
        static_cast<std::uint8_t>(0xC0U | (static_cast<std::uint32_t>(sourceCode) << 3U) |
...
void Emitter::appendModRm(std::uint8_t regField, Register rm) {
    ...
    const auto rmCode = static_cast<std::uint32_t>(registerCode(rm));
    const auto modRm =
        static_cast<std::uint8_t>(0xC0U | ((regCode & 0x7U) << 3U) | (rmCode & 0x7U));
    appendByte(modRm);
}
} // namespace ember::jit::x64
