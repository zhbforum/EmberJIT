    ...
    ember::bytecode::Program program{
        .functions = {{.id = 0,
                       ...
                       .code = {{.opcode = ember::bytecode::Opcode::constant,
                                ...
                                {.opcode = ember::bytecode::Opcode::store,
                                 .operand = 1,
                                 .value = std::nullopt},
                                {.opcode = ember::bytecode::Opcode::load,
                                 .operand = 1,
                                 .value = std::nullopt},
                                {.opcode = ember::bytecode::Opcode::load,
                                 .operand = 0,
                                 .value = std::nullopt},
                                {.opcode = ember::bytecode::Opcode::lessI64,
                                 .operand = 0,
                                 .value = std::nullopt},
                                ...
                                {.opcode = ember::bytecode::Opcode::addI64,
                                 .operand = 0,
                                 .value = std::nullopt},
                                {.opcode = ember::bytecode::Opcode::store,
                                 .operand = 1,
...
int main()
{
    ...
    return 0;
#else
    const auto vm = medianNanosecondsPerCall(
        {.hotThreshold = 1, .jitEnabled = false, .profilingEnabled = true}, false);
    const auto jit = medianNanosecondsPerCall(
        {.hotThreshold = 1, .jitEnabled = true, .profilingEnabled = true}, true);
    ...
    std::cout << "speedup: " << vm / jit << "x; latency reduction: "
              << (1.0 - jit / vm) * 100.0 << "%\n";
    return 0;
#endif
}
